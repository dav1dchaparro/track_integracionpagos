...
def _setup(client, email="filter@example.com"):
...
def test_filter_by_product(client):
    headers, _ = _setup(client, "filt1@example.com")
    resp = client.get("/sales/?filter_product=Coffee", headers=headers)
...
def test_filter_by_category(client):
    headers, _ = _setup(client, "filt2@example.com")
    resp = client.get("/sales/?filter_category=drinks", headers=headers)
    assert resp.status_code == 200
    data = resp.json()
    assert len(data) == 3
...
def test_filter_by_multiple_fields(client):
    headers, _ = _setup(client, "filt3@example.com")
    resp = client.get("/sales/?filter_product=Coffee&filter_quantity=5", headers=headers)
    assert resp.status_code == 200
    data = resp.json()
    assert len(data) == 1
...
def test_filter_no_match(client):
    headers, _ = _setup(client, "filt4@example.com")
    resp = client.get("/sales/?filter_product=Pizza", headers=headers)
    assert resp.status_code == 200
