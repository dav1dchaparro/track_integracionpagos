...
def test_create_sale(client):
    headers, template_id = _setup(client, "sale1@example.com")
...
def test_create_sale_missing_required(client):
    headers, template_id = _setup(client, "sale2@example.com")
    resp = client.post("/sales/", json={
        "template_id": template_id,
        "data": {"product": "Coffee"},
        "sold_at": datetime.now(timezone.utc).isoformat(),
...
def test_create_sale_wrong_type(client):
    headers, template_id = _setup(client, "sale3@example.com")
    resp = client.post("/sales/", json={
        "template_id": template_id,
...
def test_create_sale_extra_field(client):
    headers, template_id = _setup(client, "sale4@example.com")
    resp = client.post("/sales/", json={
        "template_id": template_id,
...
def test_list_sales(client):
    headers, template_id = _setup(client, "sale5@example.com")
    for i in range(3):
